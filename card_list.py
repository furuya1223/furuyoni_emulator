...
CARD_LIST[Goddess.UTSURO_HAJIMARI][0][1].append(
    [AttackCard(Goddess.UTSURO_HAJIMARI, '数多ノ刃', Attack([1, 2], 4, 3), trump=True, cost=5,
                image_filename='na_00_hajimari_a_s_1.png')])
CARD_LIST[Goddess.UTSURO_HAJIMARI][0][1].append(
    [ActionCard(Goddess.UTSURO_HAJIMARI, '闇凪ノ声', effects=[
        Effect(effect_type=EffectType.ACTION, content=lambda b, p: b.players[p].draw(2),
               summary='カードを2枚引く')
    ], trump=True, cost=4, image_filename='na_00_hajimari_a_s_2.png')])


def ku_no_gaitou(board, player_type):
...
CARD_LIST[Goddess.UTSURO_HAJIMARI][0][1].append(
    [ActionCard(Goddess.UTSURO_HAJIMARI, '苦ノ外套', effects=[
        ...
        Effect(effect_type=EffectType.ACTION,
               content=lambda b, p: Area.move_flowers(b.players[p.opponent].aura, b.dust, 2),
               summary='相オーラ→ダスト(2)')
    ], counter=True, trump=True, cost=4, image_filename='na_00_hajimari_a_s_3.png')])


# ユリナ
CARD_LIST[Goddess.YURINA] = [[[], []], [[], []]]
# 斬
CARD_LIST[Goddess.YURINA][0][0].append([AttackCard(1, '斬', Attack([3, 4], 3, 1),
                                        image_filename='na_01_yurina_o_n_1.png')])

# 足捌き
CARD_LIST[Goddess.YURINA][0][0].append([ActionCard(1, '足捌き', effects=[
        Effect(effect_type=EffectType.ACTION, condition=lambda b, p: b.distance() >= 4,
               content=lambda b, p: b.move_flower(AreaType.DISTANCE, AreaType.DUST, 2, p),
               summary='間合いが4以上なら 間合→ダスト(2)'),
        Effect(effect_type=EffectType.ACTION, condition=lambda b, p: b.distance() <= 1,
...
# 月影落
CARD_LIST[Goddess.YURINA][0][1].append([AttackCard(1, '月影落', Attack([3, 4], 4, 4), trump=True, cost=7)])
